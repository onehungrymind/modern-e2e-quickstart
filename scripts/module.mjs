...
function validateModule(arg) {
  ...
  if (!MODULES.includes(n)) {
    console.error(`Unknown module "${arg}". Modules are 00..12.`);
    process.exit(1);
  }
  return n;
}
...
function ensureTagExists(tag) {
  const exists = tryRun(`git rev-parse --verify --quiet refs/tags/${tag}`);
  if (!exists) {
    console.error(`Tag '${tag}' does not exist. Run scripts/rebuild-tags.mjs or git fetch --tags.`);
    process.exit(1);
  }
}
...
function begin(n) {
  ...
  const branchExists = tryRun(`git rev-parse --verify --quiet refs/heads/${branch}`);
  if (branchExists) {
...
function compare(n) {
  ...
  console.log('---');
  try {
    execSync(`git diff ${completeTag} -- apps/web-e2e`, { stdio: 'inherit' });
  } catch {
    /* git diff returns non-zero when there is a diff */
  }
}
...
function reset(n) {
  ...
  if (current !== branch) {
    console.error(`Currently on '${current}', not '${branch}'. Switch to ${branch} first or run module:begin ${n}.`);
    process.exit(1);
  }
...
switch (command) {
  case 'begin':
    ...
    break;
  case 'compare':
    compare(validateModule(moduleArg));
    break;
  case 'reset':
  ...
  case 'status':
    ...
    break;
  default:
    console.error('Usage: node scripts/module.mjs <begin|compare|reset|status> [NN]');
    process.exit(1);
}
